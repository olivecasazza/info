...
impl WebHandle {
    ...
    pub fn set_pipe_count(&self, v: usize) {
        EXTERNAL_CONFIG.with(|c| c.borrow_mut().pipe_count = v.clamp(1, 8));
        mark_config_dirty();
    }
    ...
    pub fn set_min_spacing(&self, v: i32) {
        EXTERNAL_CONFIG.with(|c| c.borrow_mut().min_spacing = v.clamp(0, 2));
        mark_config_dirty();
    }
    ...
    pub fn set_straightness(&self, v: u32) {
        EXTERNAL_CONFIG.with(|c| c.borrow_mut().straightness = v.clamp(1, 20));
        mark_config_dirty();
    }
    ...
    pub fn set_max_len_per_pipe(&self, v: usize) {
        EXTERNAL_CONFIG.with(|c| c.borrow_mut().max_len_per_pipe = v.clamp(10, 2000));
        mark_config_dirty();
    }
...
fn sync_external_config(mut state: ResMut<PipedreamState>) {
    ...
    if ext.pipe_count != state.pipe_count {
        ...
        let count = state.pipe_count;
        state.sim.reset(count);
    }
...
fn draw_pipes(state: &PipedreamState, painter: &egui::Painter, rect: Rect) {
    ...
    // Project function
    let iso_centered =
        |x: f32, y: f32, z: f32| -> Pos2 { state.renderer.project(x, y, z) + center.to_vec2() };
...
fn ui_system(
    ...
    ui.frame(ctx, dt, |egui_ui| {
        egui_ui.collapsing("simulation", |ui| {
            ...
            ui.add(
                egui::Slider::new(&mut state.sim.max_len_per_pipe, 10..=2000).text("max length"),
            );
